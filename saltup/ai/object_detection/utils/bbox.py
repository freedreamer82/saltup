...
def compute_matrices_overlap(
    ...
    Args:
        boxes1: First set of bounding boxes as a numpy array.
        boxes2: Second set of bounding boxes as a numpy array.
    ...
    enclose_wh = np.maximum(enclose_maxes - enclose_mins, 0.)
    ...
    diagonal_length = np.sum(np.square(enclose_wh), axis=-1)
    ...
    if iou_type == IoUType.CIOU:
        v = (4 / (np.pi ** 2)) * np.square(
            np.arctan(b1_wh[..., 0] / (b1_wh[..., 1] + eps)) -
            np.arctan(b2_wh[..., 0] / (b2_wh[..., 1] + eps))
        )
        alpha = v / (1 - iou + v + eps)
        ciou = iou - (center_distance / (diagonal_length + eps) + alpha * v)
        return ciou

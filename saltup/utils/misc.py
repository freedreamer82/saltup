...
def remove_prefix_from_files(folder_path: str, prefix: str, extensions: tuple[str] = ("jpg", "png", "txt")) -> None:
    """Remove prefix from filenames with specified extensions.
    
    Args:
        folder_path: Directory containing files
        prefix: String to remove from start of filenames
        extensions: File extensions to process
    ...
    for filename in os.listdir(folder_path):
        if filename.startswith(prefix) and filename.endswith(extensions):
            name, ext = os.path.splitext(filename)

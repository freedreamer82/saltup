...
import tempfile
import torch
from saltup.ai.utils.torch.to_onnx import convert_pytorch_to_onnx
...
def test_convert_pytorch_to_onnx_success():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        result = convert_pytorch_to_onnx(model, input_shape, output_path)
...
def test_convert_pytorch_to_onnx_invalid_model():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        output_path = os.path.join(tmpdir, "dummy.onnx")
        result = convert_pytorch_to_onnx(None, input_shape, output_path)
